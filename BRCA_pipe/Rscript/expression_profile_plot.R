...
normal_expression <- GTEx_data[rownames(GTEx_data) %in% gene_id$ensembl_gene_id, ]
...
expression_df <- data.frame(LumA = apply(LumA_expression, 1, mean),
                            ...
                            normal = apply(normal_expression, 1, mean))
...
ggplot(expression_df, aes(x = variable, y = value, fill = Gene)) +
  ...
  labs(x = "Cancer Type", y = "Expression Level", fill = "Gene") +
  theme_minimal() +
