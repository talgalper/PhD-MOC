...
selected_barcodes <- c(Her2_paired$normal, Her2_paired$Her2)
...
Her2_data <- GDCprepare(Her2_query, summarizedExperiment = T)


Her2_unstranded <- assay(Her2_data, "unstranded")  
rownames(Her2_unstranded) <- gsub("\\.\\d+", "", rownames(Her2_unstranded))
Her2_unstranded <- as.data.frame(Her2_unstranded)



subtype_subset <- master[master$cases %in% colnames(Her2_unstranded), ]
subtype_subset <- subtype_subset[match(colnames(Her2_unstranded), subtype_subset$cases), ]

group <- factor(subtype_subset$Subtype_Selected)
pat_id <- factor(subtype_subset$bcr_patient_barcode)

counts_filt <- filterByExpr(Her2_unstranded, group = group)
counts_filt <- Her2_unstranded[counts_filt, ]

low_exp_genes <- Her2_unstranded[!counts_filt, ]
...
write.table(gene_data$external_gene_name, "intermediate/paired/Her2/gene_list.txt", quote = F, row.names = F, col.names = F)


#### get interaction data ####

string_edge_data <- read.table("intermediate/paired/Her2/STRING network (physical) default edge.csv", header = T, sep = ",", stringsAsFactors = F)
...
string_node_data <- read.table("intermediate/paired/Her2/STRING network (physical) default node.csv", header = T, sep = ",", stringsAsFactors = F)
...
save(final_df, gene_data, file = "RData/paired/Her2/PCSF_input.RData")
...
save(subnet, file = "RData/paired/Her2/PCSF_subnet.RData")
...
write.csv(df, "intermediate/paired/Her2/PCSF_output.csv", row.names = F)
...
write.csv(PCSF_results, "intermediate/paired/Her2/PCSF_druggability.csv")


# run citation scoring tool and read in
citation_scores <- read.csv("intermediate/Her2/citation_scores.csv")
...
write.csv(filtered_results, "intermediate/Her2/PCSF_master_unique.csv")
...
write.csv(final_gene_counts, "intermediate/paired/Her2/final_gene_counts.csv", row.names = F)
