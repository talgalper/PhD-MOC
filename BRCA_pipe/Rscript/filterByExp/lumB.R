...
dir.create("intermediate/lumB/filterByExp")
dir.create("RData/lumB/filterByExp")
...
disease_df <- data.frame(sample = colnames(LumB_unstranded),
                         subtype = rep("disease", ncol(LumB_unstranded)))
...
merged_df <- merge(normal_unstranded, LumB_unstranded, by = "row.names")
...
write.csv(hits, "intermediate/lumB/filterByExp/DE_results.csv", row.names = F)


# compare DE results with hard threshold expression filter pipeline
custom_filt_DE_results <- read.csv("intermediate/lumB/DE_results.csv")
...
venn.diagram(
  ...
  filename = "intermediate/lumB/filterByExp/DE_result_compare.png",
...
write.table(gene_data$external_gene_name, "intermediate/lumB/filterByExp/gene_list.txt", quote = F, row.names = F, col.names = F)


#### get interaction data ####

string_edge_data <- read.table("intermediate/lumB/filterByExp/STRING network (physical) default edge.csv", header = T, sep = ",", stringsAsFactors = F)
...
string_node_data <- read.table("intermediate/lumB/filterByExp/STRING network (physical) default node.csv", header = T, sep = ",", stringsAsFactors = F)
...
save(final_df, gene_data, file = "RData/lumB/filterByExp/PCSF_input.RData")


# compare STRING outputs
custom_filt_string_node <- read.csv("intermediate/lumB/STRING network (physical) default node.csv")
...
venn.diagram(
  ...
  cex = 1.5,  # increase the size of the circles
  margin = 0.1,  # set the margin size (proportion of the plot)
  filename = "intermediate/lumB/filterByExp/string_node_compare.png",
  disable.logging = TRUE
)

load("RData/lumB/PCSF_input.RData")
...
venn.diagram(
  ...
  cex = 1.5,  # increase the size of the circles
  margin = 0.1,  # set the margin size (proportion of the plot)
  filename = "intermediate/lumB/filterByExp/string_edge_compare.png",
  disable.logging = TRUE
)
...
save(subnet, file = "RData/lumB/filterByExp/PCSF_subnet.RData")
...
write.csv(df, "intermediate/lumB/filterByExp/PCSF_output.csv", row.names = F)
...
write.csv(PCSF_results, "intermediate/lumB/filterByExp/PCSF_druggability.csv")
...
write.csv(filtered_results, "intermediate/lumB/filterByExp/PCSF_master_unique.csv")
...
write.csv(final_gene_counts, "intermediate/lumB/filterByExp/final_gene_counts.csv", row.names = F)


custom_filt_final_counts <- read.csv("intermediate/lumB/final_gene_counts.csv")
venn.diagram(
