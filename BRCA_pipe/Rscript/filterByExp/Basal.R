...
dir.create("intermediate/basal/filterByExp")
dir.create("RData/basal/filterByExp")
...
disease_df <- data.frame(sample = colnames(Basal_unstranded),
                         subtype = rep("disease", ncol(Basal_unstranded)))
...
merged_df <- merge(normal_unstranded, Basal_unstranded, by = "row.names")
...
write.csv(hits, "intermediate/basal/filterByExp/DE_results.csv", row.names = F)


# compare DE results with hard threshold expression filter pipeline
custom_filt_DE_results <- read.csv("intermediate/basal/DE_results.csv")
...
venn.diagram(
  ...
  filename = "intermediate/basal/filterByExp/DE_result_compare.png",
...
write.table(gene_data$external_gene_name, "intermediate/basal/filterByExp/gene_list.txt", quote = F, row.names = F, col.names = F)


#### get interaction data ####

string_edge_data <- read.table("intermediate/basal/filterByExp/STRING network (physical) default edge.csv", header = T, sep = ",", stringsAsFactors = F)
...
string_node_data <- read.table("intermediate/basal/filterByExp/STRING network (physical) default node.csv", header = T, sep = ",", stringsAsFactors = F)
...
save(final_df, gene_data, file = "RData/basal/filterByExp/PCSF_input.RData")


# compare STRING outputs
custom_filt_string_node <- read.csv("intermediate/basal/STRING network (physical) default node.csv")
...
venn.diagram(
  ...
  cex = 1.5,  # increase the size of the circles
  margin = 0.1,  # set the margin size (proportion of the plot)
  filename = "intermediate/basal/filterByExp/string_node_compare.png",
  disable.logging = TRUE
)

load("RData/basal/PCSF_input.RData")
custom_filt_STRING_edge <- final_df # make sure to rerun STRING network and gene_data code after this
...
venn.diagram(
  ...
  cex = 1.5,  # increase the size of the circles
  margin = 0.1,  # set the margin size (proportion of the plot)
  filename = "intermediate/basal/filterByExp/string_edge_compare.png",
  disable.logging = TRUE
)
...
save(subnet, file = "RData/basal/filterByExp/PCSF_subnet.RData")
...
write.csv(df, "intermediate/basal/filterByExp/PCSF_output.csv", row.names = F)
...
write.csv(PCSF_results, "intermediate/basal/filterByExp/PCSF_druggability.csv")
...
write.csv(filtered_results, "intermediate/basal/filterByExp/PCSF_master_unique.csv")
...
write.csv(final_gene_counts, "intermediate/basal/filterByExp/final_gene_counts.csv", row.names = F)


custom_filt_final_counts <- read.csv("intermediate/basal/final_gene_counts.csv")
venn.diagram(
  ...
  cex = 1.5,  # increase the size of the circles
  margin = 0.1,  # set the margin size (proportion of the plot)
  filename = "intermediate/basal/filterByExp/final_gene_counts_compare.png",
  disable.logging = TRUE
)
