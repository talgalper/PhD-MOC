...
selected_barcodes <- common[common$Subtype_Selected == "BRCA.LumB", ]
...
colnames(LumB_unstranded) <- paste("LumB", 1:ncol(LumB_unstranded), sep = "_")
colnames(normal_unstranded) <- paste("normal", 1:ncol(normal_unstranded), sep = "_")

merged_df <- merge(LumB_unstranded, normal_unstranded, by = "row.names")
...
cols <- grep("^LumB", colnames(merged_df))
...
write.table(gene_data$external_gene_name, "intermediate/LumB/gene_list.txt", quote = F, row.names = F, col.names = F)


#### get interaction data ####

string_edge_data <- read.table("intermediate/LumB/STRING network (physical) default edge.csv", header = T, sep = ",", stringsAsFactors = F)
...
string_node_data <- read.table("intermediate/LumB/STRING network (physical) default node.csv", header = T, sep = ",", stringsAsFactors = F)
...
save(final_df, gene_data, file = "RData/LumB/PCSF_input.RData")
...
save(subnet, file = "RData/LumB/PCSF_subnet.RData")
...
write.csv(df, "intermediate/LumB/PCSF_output.csv", row.names = F)
...
write.csv(PCSF_results, "intermediate/LumB/PCSF_druggability.csv")
...
write.csv(final_gene_counts, "intermediate/LumB/final_gene_counts.csv", row.names = F)
