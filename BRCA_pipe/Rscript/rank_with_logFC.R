...
# make sure to double check which data is being read in
...
logFC_norm <- (PCSF_master$logFC - min(PCSF_master$logFC)) / (max(PCSF_master$logFC) - min(PCSF_master$logFC))
...
logFC_w_values <- seq(0, 1, length.out = num_weights)
...
counts_when_betweeness_0 <- sensitivity_results[
  sensitivity_results$betweeness_weight == 0 &
    rowSums(sensitivity_results[, c("centrality_weight", "druggability_weight", "cryptic_pocket_weight", "logFC_weight")]) != 0, ]
...
counts_when_centrality_0 <- sensitivity_results[
  sensitivity_results$centrality_weight == 0 &
    rowSums(sensitivity_results[, c("betweeness_weight", "druggability_weight", "cryptic_pocket_weight", "logFC_weight")]) != 0, ]
...
counts_when_druggability_0 <- sensitivity_results[
  sensitivity_results$druggability_weight == 0 &
    rowSums(sensitivity_results[, c("centrality_weight", "betweeness_weight", "cryptic_pocket_weight", "logFC_weight")]) != 0, ]
...
counts_when_cryptic_pockets_0 <- sensitivity_results[
  sensitivity_results$cryptic_pocket_weight == 0 &
    rowSums(sensitivity_results[, c("centrality_weight", "betweeness_weight", "druggability_weight", "logFC_weight")]) != 0, ]
...
counts_when_logFC_0 <- sensitivity_results[
  sensitivity_results$logFC_weight == 0 &
    rowSums(sensitivity_results[, c("centrality_weight", "betweeness_weight", "druggability_weight", "cryptic_pocket_weight")]) != 0, ]
counts_when_logFC_0 <- sapply(all_genes_unique, function(g) sum(sapply(counts_when_logFC_0$top_genes_list, function(lst) g %in% lst)))
gene_counts <- cbind(gene_counts, counts_when_logFC_0)
...
save(final_gene_counts, file = "RData/lumA/rank_sensitivity_with_logFC.RData")
