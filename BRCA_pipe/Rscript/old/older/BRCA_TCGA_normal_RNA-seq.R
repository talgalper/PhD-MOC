...
selected_barcodes <- selected_barcodes[selected_barcodes$Subtype_Selected == "BRCA.Normal", ]


Normal_query <- GDCquery(project = "TCGA-BRCA",
                       access = "open", 
                       data.category = "Transcriptome Profiling",
...
Normal_data <- GDCprepare(Normal_query, summarizedExperiment = T)


TCGA_normal_unstranded <- assay(Normal_data, "unstranded")  
rownames(TCGA_normal_unstranded) <- gsub("\\.\\d+", "", rownames(TCGA_normal_unstranded))
TCGA_normal_unstranded <- as.data.frame(TCGA_normal_unstranded)
...
save(TCGA_TCGA_normal_unstranded, GTEx_data, file = "RData/Normal/BRCA_Normal_DE_data.RData")

barplot(colSums(TCGA_normal_unstranded),
        xaxt = "n")
...
merged_df <- merge(TCGA_normal_unstranded, GTEx_data, by = "row.names")
...
save(dif_exp, hits, file = "RData/Normal/DE_results.RData")
...
write.table(gene_data$external_gene_name, "intermediate/Normal/gene_list.txt", quote = F, row.names = F, col.names = F)


#### get interaction data ####

string_edge_data <- read.table("intermediate/Normal/STRING network (physical) default edge.csv", header = T, sep = ",", stringsAsFactors = F)
...
string_node_data <- read.table("intermediate/Normal/STRING network (physical) default node.csv", header = T, sep = ",", stringsAsFactors = F)
...
save(final_df, gene_data, file = "RData/Normal/PCSF_input.RData")
...
save(subnet, file = "RData/Normal/PCSF_subnet.RData")
...
write.csv(PCSF_results, "intermediate/Normal/PCSF_druggability.csv")


# run citation scoring tool and read in
citation_scores <- read.csv("intermediate/Normal/citation_scores.csv")
...
write.csv(filtered_results, "intermediate/Normal/PCSF_master_unique.csv")
