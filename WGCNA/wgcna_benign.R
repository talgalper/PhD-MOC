...
gsg <- goodSamplesGenes(t(benign_data))
summary(gsg)
gsg$allOK

# This method visualises outliers as cluster tree
htree <- hclust(dist(t(benign_data)))
sizeGrWindow(8, 6)
plot(htree, xlab = "", sub = "")

# PCA
pca <- prcomp(t(benign_data))
pca_data <- pca$x
pca_var <- pca$sdev^2
...
outliers <- c("GAMuT_23091", "GAMuT_41828")

benign_data_subset <- benign_data[, !colnames(benign_data) %in% outliers]


## Normalisation

# log transformation normalisation
data_log_norm <- log(benign_data_subset + 1)
...
wgcna_ben_data <- subset(data_log_norm, !(rownames(data_log_norm) %in% gene_names[failed_genes]))

wgcna_ben_data <- t(wgcna_ben_data)
...
sft <- pickSoftThreshold(wgcna_ben_data,
                         powerVector = power,
                         networkType = "signed",
                         verbose = 5)
...
wgcna_ben_data[] <- sapply(wgcna_ben_data, as.numeric)
...
bwnet_ben <- blockwiseModules(wgcna_ben_data,
                          maxBlockSize = 15000,
                          TOMType = "signed",
                          power = soft_power,
                          mergeCutHeight = 0.25,
...
module_eigengenes <- bwnet_ben$MEs

table(bwnet_ben$colors)

# Plot the dendrogram and the module colors before and after merging underneath
plotDendroAndColors(bwnet_ben$dendrograms[[1]], cbind(bwnet_ben$unmergedColors, bwnet_ben$colors),
                    c("unmerged", "merged"),
                    dendroLabels = FALSE,
                    addGuide = TRUE,
                    hang= 0.03,
...
## clean up sample_info and filter for samples in wgcna_ben_data
...
#
#wgcna_samples <- rownames(wgcna_ben_data)
#
...
#
#traits <- binarizeCategoricalColumns(sample_info_filtered$Stage,
#                                     includePairwise = FALSE,
#                                     includeLevelVsAll = TRUE,
#                                     minCount = 1)
#
#
#rownames(traits) <- sample_info_filtered$GAMUT_ID
#
#
# Define numbers of genes and samples
nSamples <- nrow(wgcna_ben_data)
nGenes <- ncol(wgcna_ben_data)
#
#
...
#             x = names(heatmap_data)[11:13],
#             y = names(heatmap_data)[1:10],
#             col = c("blue1", "skyblue", "white", "pink", "red"))
#
#
#
#module_gene_mapping <- as.data.frame(bwnet_ben$colors)
#module_gene_mapping %>% 
#  filter(`bwnet_ben$colors` == 'pink') %>% 
#  rownames()
...
module_membership_measure <- cor(module_eigengenes, wgcna_ben_data, use = 'p')
...
#gene_signf_corr <- cor(wgcna_ben_data, traits$data.IV.vs.all, use = 'p')
...
save(bwnet_ben, wgcna_ben_data, file = "benign_results/WGCNA_benign.RData")
