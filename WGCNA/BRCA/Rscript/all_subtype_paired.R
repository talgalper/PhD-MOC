...
# combine all tumour samples and subset paired
paired_tumour <- c(lumA_paired$lumA, lumB_paired$lumB, Her2_paired$Her2, Basal_paired$Basal)
paired_normal <- c(lumA_paired$normal, lumB_paired$normal, Her2_paired$normal, Basal_paired$normal)

all_subtypes <- cbind(LumA_unstranded, LumB_unstranded, Her2_unstranded, Basal_unstranded)
all_subtypes <- all_subtypes[, colnames(all_subtypes) %in% paired_tumour]

control_paired <- normal_unstranded[, colnames(normal_unstranded) %in% paired_normal]
...
# plot PCA
control_info <- data.frame(sample = colnames(control_paired),
                           group = rep("control", ncol(control_paired)))
lumA_info <- data.frame(sample = lumA_paired$lumA,
                        group = rep("lumA", length(lumA_paired$lumA)))
lumB_info <- data.frame(sample = lumB_paired$lumB,
                        group = rep("lumB", length(lumB_paired$lumB)))
her2_info <- data.frame(sample = Her2_paired$Her2,
                        group = rep("Her2", length(Her2_paired$Her2)))
basal_info <- data.frame(sample = Basal_paired$Basal,
                         group = rep("basal", length(Basal_paired$Basal)))
