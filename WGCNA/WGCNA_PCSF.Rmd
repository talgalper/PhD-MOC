...
The run took approximately 30hrs with 10 iterations, adding random noise each time. PCSF was run with the following parameters/function:
...
The next section of this document is dedicated to the raning of genes based on a combination of scoring methods. This ranking system can be easily interchangeable with other/new data types. Here we also include the addition of cryptic pocket scores which were analysed in python using the same AlphaFold structures as done with Fpocket. Any structures less than 80 amino acids in length were removed per the PocketMiner methodology. Using a 10 amino acid "window" average, pockets are calculated per amino acid.
...
If interested can view correlation plots between druggability and cryptic pockets can be viewed here:
