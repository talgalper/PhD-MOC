...
```{r load data and remove low activity genes, include=FALSE}
# combine stages
...
In this pipeline MOC RNA-Seq data is analysed using WGCNA to obtain two separate co-expression networks for benign and disease groups. Several filtering steps were undertaken in order to reduce noise:
...
-   The adjacency function was then utilised from the WGCNA which calculates (correlation or distance) network adjacency from given expression data
...
PCSF_results_unique <- PCSF_results[!duplicated(PCSF_results$external_gene_name), ]

# add gene description
gene_description <- getBM(attributes = c("external_gene_name", "description"), 
                          filters = "external_gene_name", 
                          values = PCSF_results_unique$external_gene_name, 
                          mart = ensembl)
gene_description <- gene_description[!duplicated(gene_description$external_gene_name), ]

PCSF_results_unique <- merge(gene_description, PCSF_results_unique, by = "external_gene_name")

PCSF_results_unique <- PCSF_results_unique[order(-PCSF_results_unique$druggability), ]

PCSF_results_unique$description <- gsub("\\s*\\[.*?\\]", "", PCSF_results_unique$description)

rownames(PCSF_results_unique) <- NULL

kable(PCSF_results_unique, format = "html", caption = "PCSF results") %>%
...
The rank sensitivity test was not performed with the PocketMiner data because its not very easy to include new data types like the rank aggregation. Can be done if required but the first comparison shows they return similar results anyway so not much value in running again if rank aggregation method is far better. It also has the benefit of being accompanied by a comprehensive study for the method.
...
betweenness <- subset(PCSF_results_unique, select = c("ID", "betweenness"))
...
citation <- subset(PCSF_results_unique, select = c("ID", "citation_score"))
...
druggability_data <- subset(PCSF_results_unique, select = c("ID", "druggability", "num_drug_pockets"))
...
temp <- subset(PCSF_results_unique, select = c("external_gene_name", "ID"))
...
aggregate_ranks <- merge(gene_description, aggregate_ranks, by = "external_gene_name")

aggregate_ranks <- aggregate_ranks[order(aggregate_ranks$Score), ]

aggregate_ranks$description <- gsub("\\s*\\[.*?\\]", "", aggregate_ranks$description)
...
betweeness_norm <- (PCSF_results_unique$betweenness - min(PCSF_results_unique$betweenness)) / (max(PCSF_results_unique$betweenness) - min(PCSF_results_unique$betweenness))

centrality_norm <- (PCSF_results_unique$degree_centrality - min(PCSF_results_unique$degree_centrality)) / (max(PCSF_results_unique$degree_centrality) - min(PCSF_results_unique$degree_centrality))

# log transformation
citation_norm <- log(PCSF_results_unique$citation_score + 1)
      ...
      for (druggability_w in druggability_w_values) {
        ...
        if (betweeness_w + citation_w + centrality_w + druggability_w == 1) {
          ...
          combined_score <- betweeness_w * betweeness_norm +
            ...
            druggability_w * PCSF_results_unique$druggability -
            citation_w * citation_norm
          
          # Rank the genes based on the combined score
          PCSF_results_edit <- PCSF_results_unique
...
gene_counts <- merge(gene_description, gene_counts, by.x = "external_gene_name", by.y = "gene")

gene_counts <- gene_counts[order(-gene_counts$count), ]

gene_counts$description <- gsub("\\s*\\[.*?\\]", "", gene_counts$description)
...
centrality <- subset(PCSF_results_unique, select = c("ID", "degree_centrality"))
centrality <- distinct(centrality)
centrality$degree_centrality <- (centrality$degree_centrality - min(centrality$degree_centrality)) / (max(centrality$degree_centrality) - min(centrality$degree_centrality))
centrality <- centrality[order(centrality$degree_centrality, decreasing = T), ]
centrality <- centrality$ID
...
pocketminer_data <- pocketminer_data[pocketminer_data$ID %in% PCSF_results_unique$ID, ]
...
PCSF_results_updated <- PCSF_results_unique[!(PCSF_results_unique$ID %in% IDs), ]
...
Enrichment is performed using the enrichR package. The plots here show the highest returning terms for the genes present in the set.
...
```{r top 20 enrichment, echo=FALSE}
aggregate_ranks_subset <- aggregate_ranks[1:20, ]


GO_enrichment <- enrichr(aggregate_ranks_subset$external_gene_name, databases = c("GO_Cellular_Component_2023", 
                                                   "GO_Biological_Process_2023",
                                                   "GO_Molecular_Function_2023",
                                                   "KEGG_2021_Human",
                                                   "Proteomics_Drug_Atlas_2023",
