...
CNV_loss <- MOC_CNV_data[MOC_CNV_data$Event == c("CN Loss", "Homozygous Copy Loss"), ]
...
CNV_loss$stage <- consolidate_stages(CNV_loss$Stage)

# Replace empty strings with NA in the Stage column
CNV_loss$stage[CNV_loss$stage == ""] <- NA

# remove any rows with no gene terms
CNV_loss <- CNV_loss[CNV_loss$Gene.Symbols != "", ]

# Split merged gene symbols
split_genes <- strsplit(CNV_loss$Gene.Symbols, ", ")
...
result_df_subset <- CN_loss_result_df[CN_loss_result_df$counts > 10, ]
