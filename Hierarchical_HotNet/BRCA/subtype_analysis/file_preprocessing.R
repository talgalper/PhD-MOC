...
preProcessing <- function(subtype) {
  ...
  write.table(gene_index, paste0("BRCA/subtype_analysis/", subtype, "/data/gene_index.tsv"), col.names = F, sep = "\t")
  write.table(edge_list_index, paste0("BRCA/subtype_analysis/", subtype, "/data/edge_list_index.tsv"), col.names = F, sep = "\t")
  write.table(DE_data_abs, paste0("BRCA/subtype_analysis/", subtype, "/data/logFC_scores_abs.tsv"), col.names = F, sep = "\t")
  write.table(DE_data_signed, paste0("BRCA/subtype_analysis/", subtype, "/data/logFC_scores_signed.tsv"), col.names = F, sep = "\t")
}
