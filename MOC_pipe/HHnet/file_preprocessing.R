...
# load in BRCA data
STRING_edge <- fread("../Hierarchical_HotNet/STRING_data/STRING_physical_ENSG.csv") 
...
# create score file
dif_exp <- fread("DE/MOC_DE_results.csv")
DE_data <- subset(dif_exp, select = c("ensembl_gene_id", "logFC"))
DE_data$logFC_abs <- abs(DE_data$logFC) # get absolute values
DE_data_abs <- subset(DE_data, select = c("ensembl_gene_id", "logFC_abs"))

fwrite(gene_index, "HHnet/data/gene_index.tsv", col.names = F, sep = "\t")
fwrite(edge_list_index, "HHnet/data/edge_list_index.tsv", col.names = F, sep = "\t")
fwrite(DE_data_abs, "HHnet/data/logFC_scores_abs.tsv", col.names = F, sep = "\t")
